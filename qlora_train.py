...
# from test import get_config, PEFTArguments
...
def get_opt(model, training_args):
...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    optim: str = field(default='adamw_bnb_8bit', metadata={"help": 'The optimizer to be used'})
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def format_dataset(dataset, dataset_format):
        ...
        elif dataset_format == 'mediform' or (dataset_format is None and args.dataset == 'oasst1'):
            dataset = dataset.map(lambda x: {
                'input': x['instruction'],
                'output': x['output'],
            })
    ...
    # Load dataset.
    if args.dataset_format == 'mediform':
...
def main():
    ...
    adam_bnb_optim = get_opt(model, training_args)
